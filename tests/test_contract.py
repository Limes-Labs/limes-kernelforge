...
class ContractTests(unittest.TestCase):
    def test_challenge_lists_required_protected_paths(self) -> None:
        ...
        self.assertIn("validateReplayRequest", challenge["commands"])
        self.assertIn("scripts/validate_replay_request.py", challenge["commands"]["validateReplayRequest"])
        ...
        self.assertIn("buildSubmissionBundle", challenge["commands"])
        self.assertIn("scripts/build_submission_bundle.py", challenge["commands"]["buildSubmissionBundle"])
        self.assertIn("validateSubmissionBundle", challenge["commands"])
        self.assertIn("scripts/validate_submission_bundle.py", challenge["commands"]["validateSubmissionBundle"])
        self.assertIn("buildCandidatePacket", challenge["commands"])
        self.assertIn("scripts/build_candidate_packet.py", challenge["commands"]["buildCandidatePacket"])
        self.assertIn("validateCandidatePacket", challenge["commands"])
        self.assertIn("scripts/validate_candidate_packet.py", challenge["commands"]["validateCandidatePacket"])
        ...
        self.assertEqual(verifier["replay_request_template_path"], "templates/replay-request.example.json")
